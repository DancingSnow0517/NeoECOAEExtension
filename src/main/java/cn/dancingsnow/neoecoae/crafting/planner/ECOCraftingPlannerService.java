...
public final class ECOCraftingPlannerService {
    ...
    private final ComponentPlanner componentPlanner =
            new ComponentPlanner(new AcyclicCraftingSolver(), SinglePatternGrowthCycleSolver.overBoundedSolver());
    ...
    public final class Session {
        ...
        public ECOPlanningResult plan(long amount, boolean simulation, ECOCancellation cancellation)
            ...
            try {
                ...
                var plan =
                        switch (solved.status()) {
                            case SUCCESS, MISSING_ITEMS -> bridge.success(
                                    goal,
                                    amount,
                                    simulation || solved.status() != PlanningStatus.SUCCESS,
                                    multiplePaths,
                                    solved.state());
                            case PARTIAL, CYCLE_UNRESOLVED -> bridge.partial(
                                    goal, amount, multiplePaths, solved.state());
                ...
                var result = new ECOPlanningResult(
                        ...
                        solved.cycles(),
                        solved.components(),
                        solved.executionComponentOrder(),
                ...
                result.setExactMaterials(
                        solved.state().usedAmounts(),
                        solved.state().emittedAmounts(),
                ...
                trace.addDiagnostic(new PlannerDiagnostic(
                        PlannerDiagnostic.Code.CANCELLED, "Crafting candidate calculation was cancelled"));
                ...
                ECOPlanTrace trace = new ECOPlanTrace();
                trace.addDiagnostic(new PlannerDiagnostic(
                        PlannerDiagnostic.Code.INTERNAL_ERROR, e.getClass().getSimpleName() + ": " + e.getMessage()));
                return new ECOPlanningResult(
                        PlanningStatus.INTERNAL_ERROR,
                        null,
                        trace,
                        List.of(),
                        List.of(),
         ...
         */
        private ECOPlanningResult rejectExecutionPlanFailure(
                ECOPlanningResult result, long amount, boolean simulation, long startedNanos) {
            String reason = result.executionPlanError();
            ECOPlanTrace trace = result.trace();
            trace.addDiagnostic(
                    new PlannerDiagnostic(PlannerDiagnostic.Code.INTERNAL_ERROR, "EXECUTION_PLAN_FAILED:" + reason));
            return new ECOPlanningResult(
                    PlanningStatus.INTERNAL_ERROR,
                    null,
                    trace,
