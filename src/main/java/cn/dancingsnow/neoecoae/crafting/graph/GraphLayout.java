...
public final class GraphLayout implements GraphLayoutEngine {
    ...
    public GraphLayout(Mode mode) {
        this(
                mode,
                new LayeredGraphLayout(),
                new CircularCycleLayout(),
                new CycleClusterLayout(),
                new CompactTreeLayout());
    }

    // Package-visible for backend selection tests without touching the screen.
    GraphLayout(Mode mode, GraphLayoutEngine legacy, GraphLayoutEngine circular, GraphLayoutEngine compactTree) {
        this(mode, legacy, circular, new CycleClusterLayout(), compactTree);
    }

    GraphLayout(
            Mode mode,
            GraphLayoutEngine legacy,
            GraphLayoutEngine circular,
            GraphLayoutEngine cycleCluster,
    ...
    private void reportCompactTreeFailure(ClientCraftingGraph graph, Throwable exception) {
        if (!reportedCompactTreeFailure) {
            ...
            LOGGER.warn(
                    "Compact tree layout failed for {} nodes and {} edges; falling back to legacy layout",
                    graph.nodes().size(),
                    graph.links().size(),
                    exception);
        }
