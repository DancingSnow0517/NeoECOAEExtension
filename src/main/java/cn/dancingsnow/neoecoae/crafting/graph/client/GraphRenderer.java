package cn.dancingsnow.neoecoae.crafting.graph.client;

import appeng.api.client.AEKeyRendering;
import appeng.api.stacks.AEKey;
import appeng.api.stacks.AmountFormat;
import cn.dancingsnow.neoecoae.crafting.graph.ClientCraftingGraph;
import cn.dancingsnow.neoecoae.crafting.graph.CycleCluster;
import cn.dancingsnow.neoecoae.crafting.graph.GraphLayoutSnapshot;
import cn.dancingsnow.neoecoae.crafting.graph.InterCycleFlow;
...
public final class GraphRenderer {
    ...
    static VisiblePlan collectVisible(
        ...
        return new VisiblePlan(
                layout.query(left, top, right, bottom, 180 / zoom),
                layout.queryLinks(left, top, right, bottom, 180 / zoom));
    }

    public Frame render(
            GuiGraphics graphics,
            ClientCraftingGraph graph,
            GraphLayoutSnapshot layout,
            float cameraX,
            float cameraY,
            float zoom,
            int screenWidth,
            ...
            int mouseX,
            int mouseY,
            @Nullable Integer selectedId,
            GraphProfiler profiler) {
        if (graph.isCompactTree()) {
            return compactTreeRenderer.render(
                    graphics,
                    graph,
                    layout,
                    cameraX,
    ...
    private static void drawCycleOutputArrow(
            GuiGraphics graphics,
            ClientCraftingGraph graph,
            GraphLayoutSnapshot layout,
            float cameraX,
        ...
        var cycle = graph.source().cycleGroups().stream()
                .filter(value -> value.componentId() == graph.focusedCycleId())
                .findFirst()
                .orElse(null);
        // Unsolved cycles are diagnostic structure, not executable output flow.
        if (cycle == null
                || (cycle.patternTimes().stream().noneMatch(value -> value.amount() > 0)
        ...
        var ring = layout.boxes().values().stream()
                .filter(box -> !graph.isBoundaryMaterial(box.nodeId()))
                .toList();
        float centerX = (float) ring.stream()
                .mapToDouble(GraphLayoutSnapshot.Box::centerX)
                .average()
                .orElse(output.centerX());
        float centerY = (float) ring.stream()
                .mapToDouble(GraphLayoutSnapshot.Box::centerY)
                .average()
        ...
        dx /= length;
        dy /= length;
        var start = new GraphLayoutSnapshot.Point(
                output.centerX() + dx * output.width() / 2, output.centerY() + dy * output.height() / 2);
    ...
    static @Nullable Integer cycleOutputNodeId(ClientCraftingGraph graph) {
        ...
        return root != null && root.kind() == ClientCraftingGraph.Kind.MATERIAL && !graph.isBoundaryMaterial(root.id())
                ? root.id()
                : null;
    }
    ...
    private static @Nullable Integer materialId(ClientCraftingGraph graph, AEKey key) {
        return graph.nodes().values().stream()
                ...
                .map(ClientCraftingGraph.Node::id)
                .findFirst()
                .orElse(null);
    }

    private static void drawEdgeAmount(
            GuiGraphics graphics,
            ClientCraftingGraph graph,
            ClientCraftingGraph.Link link,
            List<GraphLayoutSnapshot.Point> route,
            float cameraX,
            float cameraY,
            float zoom) {
    ...
    private static String edgeAmount(ClientCraftingGraph graph, ClientCraftingGraph.Link link) {
        ...
        AEKey key = material == null || material.material() == null
                ? null
                : material.material().key();
        if (key == null && link.materialNodeId() >= 0) {
            key = graph.source().nodes().stream()
                    .filter(node -> node.nodeId() == link.materialNodeId())
                    .map(CraftingGraphSnapshot.MaterialNode::key)
                    .findFirst()
                    .orElse(null);
        }
    ...
    private static void drawClusterLabels(
            GuiGraphics graphics,
            ClientCraftingGraph graph,
            GraphLayoutSnapshot layout,
            float cameraX,
        ...
        for (int componentId : cluster.componentIds()) {
            ...
            graph.source().cycleGroups().stream()
                    .filter(cycle -> cycle.componentId() == componentId)
                    .findFirst()
                    .ifPresent(cycle -> ids.addAll(cycle.memberNodeIds()));
            graph.nodes().values().stream()
                    .filter(node -> node.pattern() != null && node.pattern().componentId() == componentId)
                    .map(ClientCraftingGraph.Node::id)
                    .forEach(ids::add);
            ...
            graphics.drawString(
                    font,
                    "循环 #" + componentId,
                    screen(cameraX, minX, zoom),
                    screen(cameraY, minY - 14, zoom),
                    0xffe2b766,
                    false);
        }
    }

    private static void drawRoutedLink(
            GuiGraphics graphics,
            List<GraphLayoutSnapshot.Point> route,
            int color,
            float cameraX,
    ...
    private static void drawNode(
        ...
        switch (node.kind()) {
            case MATERIAL -> {
                ...
                if (zoom >= 0.7f && pixelWidth >= 82 && pixelHeight >= 48) {
                    ...
                    graphics.drawCenteredString(
                            ...
                            statusSymbol(material.status()),
                            (x + right) / 2,
                            y + Math.max(2, (pixelHeight - font.lineHeight) / 2),
                            border);
                }
            }
            case PATTERN -> {
                ...
                graphics.drawCenteredString(
                        font, symbol, (x + right) / 2, y + Math.max(3, pixelHeight >= 28 ? 6 : 2), border);
            ...
            case CYCLE_GROUP -> {
                ...
                drawFitted(graphics, font, "↻  循环 #" + cycle.componentId(), x + 7, y + 7, pixelWidth - 14, border);
                if (zoom >= 0.6f && pixelHeight >= 38) {
                    drawFitted(
                            graphics,
                            font,
                            cycle.memberNodeIds().size() + " 个成员 · " + cycle.status(),
                            x + 7,
                            y + 23,
                            pixelWidth - 14,
                            TEXT);
                    String required = cycle.requiredOutputs().isEmpty()
                            ? "变化：-"
                            : "所需产物：" + cycle.requiredOutputs().size();
                    if (pixelHeight >= 55) drawFitted(graphics, font, required, x + 7, y + 40, pixelWidth - 14, MUTED);
                    if (pixelHeight >= 68) drawFitted(graphics, font, "双击查看内部图", x + 7, y + 54, pixelWidth - 14, MUTED);
                }
            }
            case CYCLE_CLUSTER -> {
                var cluster = node.cluster();
                drawFitted(graphics, font, "◎  循环簇 #" + cluster.clusterId(), x + 7, y + 7, pixelWidth - 14, border);
                if (zoom >= 0.6f && pixelHeight >= 38) {
                    drawFitted(
                            graphics,
                            font,
                            cluster.componentIds().size() + " 个独立循环 · "
                                    + cluster.flows().size() + " 条跨环流",
                            x + 7,
                            y + 23,
                            pixelWidth - 14,
                            TEXT);
                    if (pixelHeight >= 55) drawFitted(graphics, font, "双击查看循环簇", x + 7, y + 40, pixelWidth - 14, MUTED);
                }
            }
            case FOLDER -> {}
            case REFERENCE -> {}
    ...
    private static void drawCompactCycleNode(
            GuiGraphics graphics,
            ClientCraftingGraph graph,
            ClientCraftingGraph.Node node,
            GraphLayoutSnapshot.Box box,
        ...
        if (node.kind() == ClientCraftingGraph.Kind.MATERIAL && node.material() != null) {
            ...
            graphics.drawCenteredString(
                    font, "⚒", (x + right) / 2, y + Math.max(1, (iconBottom - y - font.lineHeight) / 2), border);
        }
    ...
    private static CompactCycleValue compactCycleValue(ClientCraftingGraph graph, ClientCraftingGraph.Node node) {
        if (node.kind() == ClientCraftingGraph.Kind.PATTERN && node.pattern() != null) {
            return new CompactCycleValue(
                    compactAmount(Long.toString(node.pattern().firingCount())), TEXT);
        }
    ...
    private static void drawScaledAmount(
            GuiGraphics graphics, Font font, String amount, int left, int top, int right, int bottom, int color) {
        int availableWidth = Math.max(1, right - left - 2);
        int availableHeight = Math.max(1, bottom - top);
        float scale = Math.min(
                0.5f,
                Math.min(
                        (float) availableWidth / Math.max(1, font.width(amount)),
    ...
    private static void drawFitted(
            GuiGraphics graphics, Font font, String text, int x, int y, int maxWidth, int color) {
    ...
    private static List<Component> tooltip(ClientCraftingGraph graph, @Nullable ClientCraftingGraph.Node node) {
        ...
        if (node.kind() == ClientCraftingGraph.Kind.CYCLE_GROUP) {
            var cycle = node.cycle();
            return List.of(
                    Component.literal("循环 #" + cycle.componentId()),
                    Component.literal(cycle.status()),
        ...
        if (node.kind() == ClientCraftingGraph.Kind.CYCLE_CLUSTER) {
            var cluster = node.cluster();
            return List.of(
                    Component.literal("循环簇 #" + cluster.clusterId()),
        ...
        if (graph.view() == ClientCraftingGraph.View.CYCLE_FOCUS && node.material() != null) {
            ...
            if (cycle != null) {
                ...
                lines.add(Component.literal(
                        "任务消耗：" + compactAmount(node.material().exactConsumed())));
                lines.add(Component.literal(
                        "任务产出：" + compactAmount(node.material().exactProduced())));
            }
        }
        return lines;
    }
    ...
    private static void drawArrowHead(
        ...
        fillTriangle(graphics, tipX, tipY, baseX + normalX, baseY + normalY, baseX - normalX, baseY - normalY, color);
    }

    private static void fillTriangle(
            GuiGraphics graphics, float ax, float ay, float bx, float by, float cx, float cy, int color) {
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                ...
                if (orientation >= 0 ? one >= 0 && two >= 0 && three >= 0 : one <= 0 && two <= 0 && three <= 0) {
                    graphics.fill(x, y, x + 1, y + 1, color);
                }
            }
        }
    ...
    private static String amountText(List<CraftingGraphSnapshot.KeyAmount> values, AEKey key) {
        return values.stream()
                .filter(value -> value.key().equals(key))
                .findFirst()
                .map(value -> formatMagnitude(key, value.amount()))
                .orElse("-");
    }

    private static String relationshipText(
            ClientCraftingGraph graph, List<CraftingGraphSnapshot.Relationship> relationships) {
        if (relationships.isEmpty()) return "-";
        return relationships.stream()
                .map(relationship -> {
                    var node = graph.nodes().get(relationship.materialNodeId());
                    String label = node == null || node.key() == null
                            ? "#" + relationship.materialNodeId()
    ...
    private static BigInteger exactAmountFor(
            List<cn.dancingsnow.neoecoae.crafting.planner.snapshot.ExactKeyAmount> exactValues,
            List<CraftingGraphSnapshot.KeyAmount> fallback,
            AEKey key) {
        return exactValues.stream()
                .filter(value -> value.key().equals(key))
                .findFirst()
                .map(value -> value.amount().value())
                .orElseGet(() -> BigInteger.valueOf(amountFor(fallback, key)));
    }
