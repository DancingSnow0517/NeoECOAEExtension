...
final class GraphPortAllocator {
    ...
    GraphLayoutSnapshot.Point attach(
            GraphLayoutSnapshot.Box box, float towardX, float towardY, boolean outgoing, @Nullable Side forcedSide) {
    ...
    static GraphLayoutSnapshot.Point evenlySpacedPoint(
            GraphLayoutSnapshot.Box box, Side side, int position, int count) {
        float fraction = (position + 1f) / (count + 1f);
        return switch (side) {
            case TOP, BOTTOM -> new GraphLayoutSnapshot.Point(
                    box.x() + box.width() * fraction, side == Side.TOP ? box.y() : box.y() + box.height());
    ...
    private static GraphLayoutSnapshot.Point fixedPoint(GraphLayoutSnapshot.Box box, Side side, int slot) {
        ...
        return switch (side) {
            ...
            case BOTTOM -> new GraphLayoutSnapshot.Point(
                    box.x() + box.width() * (1 - fraction), box.y() + box.height());
