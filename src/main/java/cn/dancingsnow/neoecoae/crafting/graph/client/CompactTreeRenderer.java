...
public final class CompactTreeRenderer {
    ...
    public GraphRenderer.Frame render(
        ...
        GraphRenderer.VisiblePlan visible =
                GraphRenderer.collectVisible(layout, cameraX, cameraY, zoom, screenWidth, screenHeight);
    ...
    private static void drawRowFrames(
            GuiGraphics graphics, GraphLayoutSnapshot layout, float cameraX, float cameraY, float zoom) {
    ...
    private static List<GraphLayoutSnapshot.Point> verticalRoute(
            GraphLayoutSnapshot.Box from, GraphLayoutSnapshot.Box to) {
        ...
        return List.of(
                new GraphLayoutSnapshot.Point(startX, startY),
                new GraphLayoutSnapshot.Point(startX, middle),
                new GraphLayoutSnapshot.Point(endX, middle),
    ...
    private static void drawFolder(
        ...
        String symbol = metadata.containsCycle()
                ? "↻"
                : metadata.containsMissing() ? "!" : metadata.containsUnsupported() ? "?" : "▱";
        graphics.drawCenteredString(font, symbol, (x + right) / 2, y + 5, border);
        if (bottom - y >= 24) drawAmount(graphics, font, "+" + metadata.hiddenNodeCount(), x, y + 20, right, bottom);
    }
    ...
    private static void drawCompactContent(
        ...
        if (node.kind() == ClientCraftingGraph.Kind.MATERIAL
            ...
            if (material != null
                    && material.key() != null
                    && bottom - y >= 24
                    && (material.consumedBigInteger().signum() > 0
                            || material.producedBigInteger().signum() > 0
                            || material.requestedBigInteger().signum() > 0)) {
                drawAmount(graphics, font, flowText(material), x, y + 20, right, bottom);
            }
            ...
            if (cycle != null)
                graphics.drawString(font, cycle.memberNodeIds().size() + " 个成员", x + 6, y + 19, MUTED, false);
            ...
            graphics.drawString(font, fit(font, label, Math.max(20, right - x - 24)), x + 6, y + 5, TEXT, false);
            if (cluster != null)
                graphics.drawString(font, cluster.componentIds().size() + " 个循环", x + 6, y + 19, MUTED, false);
            graphics.drawString(font, "◎", right - 14, y + 9, border, false);
        } else if (node.kind() == ClientCraftingGraph.Kind.REFERENCE) {
            graphics.drawString(
                    font, fit(font, node.label(), Math.max(20, right - x - 10)), x + 6, y + 9, MUTED, false);
        }
    }

    private static void drawAmount(
            GuiGraphics graphics, Font font, String amount, int left, int top, int right, int bottom) {
    ...
    private static List<Component> tooltip(ClientCraftingGraph graph, @Nullable ClientCraftingGraph.Node node) {
        ...
        if (node.kind() == ClientCraftingGraph.Kind.REFERENCE) {
            ...
            return List.of(
                    ...
                    Component.literal("shared dependency reference"),
        ...
        if (node.material() != null) {
            ...
            return List.of(
                    Component.literal(node.label()),
                    Component.literal("requested: " + material.exactRequested()),
                    Component.literal("inventory: " + material.exactFromInventory()),
                    Component.literal("to craft: " + material.exactToCraft()),
                    Component.literal("missing: " + material.exactMissing()),
    ...
    private static String flowText(CraftingGraphSnapshot.MaterialNode material) {
        if (showsPendingCraftLabel(material)) {
            return Component.translatable("gui.neoecoae.crafting_graph.node.pending_craft")
                    .getString();
        }
        if (material.consumedBigInteger().signum() == 0
                && material.producedBigInteger().signum() == 0) {
