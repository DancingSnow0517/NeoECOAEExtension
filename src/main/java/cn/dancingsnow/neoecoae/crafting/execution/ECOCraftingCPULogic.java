...
public class ECOCraftingCPULogic {
    ...
    private void refillExactMaterials() {
        ...
        if (current.playerId != null) {
            ...
            var player = level == null || level.getServer() == null
                    ? null
                    : IPlayerRegistry.getConnected(level.getServer(), current.playerId);
        ...
        for (var entry : current.deferredStock.entrySet()) {
            ...
            long extracted = grid.getStorageService()
                    .getInventory()
                    .extract(entry.getKey(), wanted, Actionable.MODULATE, source);
            if (extracted > 0) {
    ...
    private void deliverStoredFinalOutput() {
        ...
        PlannerAmount deliverable = PlannerAmount.of(ECOExactInventory.amount(inventory, key))
                .subtract(reserve)
        ...
        if (current.remainingAmount <= 0L
                ...
                && current.tasks.values().stream()
                        .noneMatch(task -> task.remainingExact().signum() > 0)) {
    ...
    static int calculateOperationLimit(int coProcessors, int configuredLimit) {
        ...
        long safeConfiguredLimit = Math.min((long) NEConfig.ECO_CPU_PUSH_TICK_LIMIT_MAX, Math.max(0L, configuredLimit));
        ...
        while (job == current) {
            ...
            for (int offset = 0; offset < candidates.size(); offset++) {
                ...
                var providers = providerCursor.availableProviders(
                        pattern, () -> collectAvailableProviders(craftingService, pattern), providerCandidate -> {
                ...
                var inputs =
                        CraftingCpuHelper.extractPatternInputs(pattern, inputInventory, level, outputs, containers);
                if (inputs == null) {
                ...
                for (var provider : providers) {
                    ...
                    if (energyService.extractAEPower(power, Actionable.SIMULATE, PowerMultiplier.CONFIG)
                            < power - 0.01) {
                    ...
                    if (!ECOBatchCraftingHelper.tryExtractExact(inventory, ECOFastPathStacks.copyCounters(inputs))) {
                        // Extraction rolled back; do not push, charge energy, or advance task progress.
                        break;
                    }
                    ...
                    try {
                        ...
                        for (var container : containers) {
                            current.waitingFor.insert(
                                    container.getKey(), container.getLongValue(), Actionable.MODULATE);
                            current.timeTracker.addMaxItems(
                        ...
                        markCpuDirty();
                        acceptedInPass = true;
                        // Inputs and the candidate budget were resolved before this provider loop. After an
                        // accepted push, rebuild both before dispatching again, just as the batch path does.
                        break;
                    } finally {
    ...
    private java.util.List<ECOExecutionRuntime.DispatchCandidate> nativeDispatchCandidates(
        ...
        for (var entry : current.tasks.entrySet()) {
            if (entry.getValue().value > 0L) {
                ...
                result.add(
                        new ECOExecutionRuntime.DispatchCandidate(0, 0, entry.getKey(), entry.getValue().value, false));
            }
        }
        return result;
    }
    ...
    public void readFromNBT(CompoundTag data, HolderLookup.Provider registries) {
        ...
        ((ECOExactInventory) inventory)
                .setEnabled(data.contains("job") && data.getCompound("job").getBoolean("exactOrder"));
    ...
    private static void addSaturated(KeyCounter destination, KeyCounter source) {
        for (var entry : source) {
            destination.set(
                    entry.getKey(), NEMath.saturatingAdd(destination.get(entry.getKey()), entry.getLongValue()));
        }
    }
    ...
    public void setJobUserPaused(boolean paused) {
        setJobSuspended(paused);
    }

    public @Nullable UUID getCraftingJobId() {
