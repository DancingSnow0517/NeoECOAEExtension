...
public record LargeWorkstationOverclock(int coolingTier, int maxParallelism, int energyMultiplier) {
    ...
    public static LargeWorkstationOverclock forCurrentSettings(
            boolean overclocked, boolean activeCooling, int coolingTier) {
        if (!overclocked || !activeCooling) return NORMAL;
        LargeWorkstationOverclock profile = forCoolingTier(coolingTier);
        return profile == null ? NORMAL : profile;
    }

    @Nullable public static LargeWorkstationOverclock forCoolingTier(int coolingTier) {
    ...
    @Nullable public static LargeWorkstationOverclock fromPersisted(int coolingTier, int energyMultiplier) {
        if (coolingTier == 0 && energyMultiplier == 1) return NORMAL;
        LargeWorkstationOverclock profile = forCoolingTier(coolingTier);
