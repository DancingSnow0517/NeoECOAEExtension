...
public final class HostText {
    ...
    private static final String[] EXPANDED_BYTE_UNITS = {"", "K", "M", "G", "T", "P", "E", "Z", "Y", "R", "Q"};
    ...
    public static String preciseHugeAmount(BigInteger value) {
        ...
        int unitIndex = 0;
        BigInteger unit = BigInteger.ONE;
        while (unitIndex < EXPANDED_BYTE_UNITS.length - 1 && safe.compareTo(unit.multiply(BIG_BYTES_IN_K)) >= 0) {
            unit = unit.multiply(BIG_BYTES_IN_K);
            unitIndex++;
        }
        while (unitIndex > 0) {
        ...
        BigDecimal scaled = new BigDecimal(safe).divide(new BigDecimal(unit), 2, RoundingMode.DOWN);
    ...
    public static List<Component> exactAmountTooltip(BigInteger value, int color) {
        ...
        while (start < exact.length()) {
            ...
            lines.add(Component.literal(exact.substring(start, end)).withStyle(style -> style.withColor(color)));
            start = end;
        }
    ...
    public static UsedTotal fullTypeProgress(long used, long max) {
        return new UsedTotal(number(Math.max(0L, used)), max < 0L ? "\u221E" : number(max), Component.empty());
    }
