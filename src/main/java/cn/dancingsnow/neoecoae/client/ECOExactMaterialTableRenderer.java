...
final class ECOExactMaterialTableRenderer extends AbstractTableRenderer<CraftingGraphSnapshot.MaterialNode> {
    ...
    ECOExactMaterialTableRenderer(AEBaseScreen<?> screen, int x, int y, Predicate<AEKey> disabledCycleRequirement) {
        super(screen, x, y, 7);
    ...
    protected List<Component> getEntryDescription(CraftingGraphSnapshot.MaterialNode entry) {
        ...
        if (missing.signum() > 0) {
            lines.add(
                    entry.status() == CraftingGraphSnapshot.MaterialStatus.CYCLE
    ...
    static List<CraftingGraphSnapshot.MaterialNode> sortMaterials(List<CraftingGraphSnapshot.MaterialNode> nodes) {
        return nodes.stream()
                ...
                .sorted(Comparator.comparing(CraftingGraphSnapshot.MaterialNode::missingBigInteger)
                        .reversed()
                        .thenComparing(CraftingGraphSnapshot.MaterialNode::toCraftBigInteger, Comparator.reverseOrder())
                        .thenComparing(
                                CraftingGraphSnapshot.MaterialNode::fromInventoryBigInteger, Comparator.reverseOrder())
