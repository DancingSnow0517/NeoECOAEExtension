...
final class ECOCycleItemListRenderer {
    ...
    void render(GuiGraphics graphics, int mouseX, int mouseY, List<ECOCycleItemList.Entry> items, int scrollOffset) {
    ...
    void render(
            ...
            int mouseX,
            int mouseY,
            List<ECOCycleItemList.Entry> items,
            int scrollOffset,
        ...
        for (int index = scrollOffset; index < end; index++) {
            ...
            boolean hovered = localMouseX >= x
                    && localMouseX < x + CELL_WIDTH
                    && localMouseY >= cellY
                    && localMouseY < cellY + ROW_HEIGHT - 1;
            boolean selected = selectedComponentId != null && selectedComponentId == entry.componentId();
            (hovered || selected ? hoveredItemBackground : itemBackground)
                    .dest(x, cellY)
                    .blit(graphics);
            ...
            for (var line : lines) {
                ...
                graphics.drawString(
                        font, line, (int) ((itemX - 2 - lineWidth * 0.5f) * 2), (int) (textY * 2), textColor, false);
            ...
            if (hovered) {
                hoveredStack = new StackWithBounds(
                        new GenericStack(item, 0),
                        new Rect2i(screen.getGuiLeft() + x, screen.getGuiTop() + cellY, CELL_WIDTH, ROW_HEIGHT - 1));
    ...
    private static boolean isMissingStartupSeed(ECOCycleItemList.Entry entry) {
        return entry.solveStatus()
                        == cn.dancingsnow.neoecoae.crafting.planner.cycle.CycleSolveStatus.INSUFFICIENT_EXTERNAL_INPUT
    ...
    @Nullable ECOCycleItemList.Entry entryAt(double mouseX, double mouseY, List<ECOCycleItemList.Entry> items, int scrollOffset) {
