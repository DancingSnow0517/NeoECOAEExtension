...
public final class LayeredGraphLayout implements GraphLayoutEngine {
    ...
    public GraphLayoutSnapshot layout(ClientCraftingGraph graph, long version) {
        ...
        graph.nodes().keySet().stream().sorted().forEach(id -> layers.computeIfAbsent(
                        layer.getOrDefault(id, 0), ignored -> new ArrayList<>())
                .add(id));

        Map<Integer, Float> priorOrder = new HashMap<>();
        for (var entry : layers.entrySet()) {
            entry.getValue()
                    .sort(Comparator.<Integer>comparingDouble(id -> barycenter(graph, id, priorOrder))
                            .thenComparingInt(Integer::intValue));
            for (int i = 0; i < entry.getValue().size(); i++)
                priorOrder.put(entry.getValue().get(i), (float) i);
        }
        ...
        float x = 0;
        for (var entry : layers.entrySet()) {
            float layerWidth = (float) entry.getValue().stream()
                    .map(graph.nodes()::get)
                    .mapToDouble(GraphLayoutSnapshot::widthFor)
                    .max()
                    .orElse(GraphLayoutSnapshot.MATERIAL_WIDTH);
            float y = 0;
        ...
        return new GraphLayoutSnapshot(
                boxes, graph.links(), new GraphLayoutSnapshot.Bounds(0, 0, maxRight, maxBottom), layoutNanos, version);
    }
    ...
    private static Map<Integer, Integer> assignLayers(ClientCraftingGraph graph) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int neighbor : graph.upstream(current))
                if (!layer.containsKey(neighbor)) {
                    layer.put(neighbor, nextLayer);
                    queue.addLast(neighbor);
                }
        }
        int orphanLayer =
                layer.values().stream().mapToInt(Integer::intValue).max().orElse(0) + 1;
