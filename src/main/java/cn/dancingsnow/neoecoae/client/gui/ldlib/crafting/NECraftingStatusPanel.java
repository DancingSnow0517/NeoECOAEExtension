...
/** Mirrors the 1.21.1 Crafting Status card: overclock and active cooling. */
public final class NECraftingStatusPanel {
    ...
    public void draw(NECraftingRenderContext context, NECraftingUiState state) {
        context.draw(
                Component.translatable("gui.neoecoae.crafting.ui.status"),
                STATUS_AREA_X + 8,
                STATUS_AREA_Y + 5,
                NELDLibStyle.DARK_TEXT_PRIMARY);
        int y = STATUS_AREA_Y + 21;
        drawRow(context, Component.translatable("gui.neoecoae.crafting.ui.overclock_short"), state.overclocked(), y);
        drawRow(
                context,
                Component.translatable("gui.neoecoae.crafting.ui.cooling_short"),
                state.activeCooling(),
                y + 15);
    }
