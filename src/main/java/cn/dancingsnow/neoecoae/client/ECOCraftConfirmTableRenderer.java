...
final class ECOCraftConfirmTableRenderer extends AbstractTableRenderer<CraftingPlanSummaryEntry> {
    ...
    ECOCraftConfirmTableRenderer(AEBaseScreen<?> screen, int x, int y, Predicate<AEKey> disabledCycleRequirement) {
        super(screen, x, y, 7);
    ...
    protected List<Component> getEntryDescription(CraftingPlanSummaryEntry entry) {
        ...
        if (entry.getStoredAmount() > 0)
            lines.add(
                    GuiText.FromStorage.text(entry.getWhat().formatAmount(entry.getStoredAmount(), AmountFormat.SLOT)));
        if (entry.getMissingAmount() > 0)
            lines.add(GuiText.Missing.text(entry.getWhat().formatAmount(entry.getMissingAmount(), AmountFormat.SLOT)));
    ...
    protected List<Component> getEntryTooltip(CraftingPlanSummaryEntry entry) {
        ...
        if (entry.getStoredAmount() > 0)
            lines.add(
                    GuiText.FromStorage.text(entry.getWhat().formatAmount(entry.getStoredAmount(), AmountFormat.FULL)));
        if (entry.getMissingAmount() > 0)
            lines.add(GuiText.Missing.text(entry.getWhat().formatAmount(entry.getMissingAmount(), AmountFormat.FULL)));
        if (entry.getCraftAmount() > 0)
            lines.add(GuiText.ToCraft.text(entry.getWhat().formatAmount(entry.getCraftAmount(), AmountFormat.FULL)));
        return lines;
    }
