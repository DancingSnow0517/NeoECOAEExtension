...
/** Runtime recipe contract for the large workstation; it is not a registered recipe type. */
public record LargeWorkstationRecipe(
        ResourceLocation id, IntegratedWorkingStationRecipe display, long energy, List<GenericStack> extraInputs) {
    ...
    public boolean matchesInputs(KeyCounter inputs) {
        ...
        long[] needed = required.stream().mapToLong(input -> input.count()).toArray();
    ...
    static boolean matchesQuantities(long[] supplied, long[] needed, boolean[][] accepts) {
        long supply = 0;
        long demand = 0;
        for (long amount : supplied) {
            if (amount <= 0) return false;
            supply = Math.addExact(supply, amount);
        }
        for (long amount : needed) {
            if (amount <= 0) return false;
