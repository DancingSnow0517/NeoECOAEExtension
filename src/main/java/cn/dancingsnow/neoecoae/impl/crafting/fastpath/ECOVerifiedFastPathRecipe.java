...
public final class ECOVerifiedFastPathRecipe {
    ...
    @Nullable public ECODurabilityBatchModel durabilityModel() {
        return result.durabilityModel();
    }

    @Nullable public ECOReusableStateModel reusableStateModel() {
        return result.reusableStateModel();
    }
    ...
    public long arithmeticBatchLimit() {
        ...
        return reusableStateModel() == null
                ? limit
                : Math.min(limit, reusableStateModel().maxBatchSize());
    }
     ...
     */
    @Nullable public ECOVerifiedFastPathExecution withBatch(int batchSize, @Nullable UUID craftingJobId) {
    ...
    public ECOVerifiedVirtualExecution withVirtualBatch(long craftCount, @Nullable UUID craftingJobId) {
        return craftCount <= 0L || craftCount > arithmeticBatchLimit() || !batchSafe()
                ? null
                : new ECOVerifiedVirtualExecution(this, craftCount, craftingJobId);
    }
}
