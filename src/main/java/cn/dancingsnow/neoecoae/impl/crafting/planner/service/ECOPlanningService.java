...
public final class ECOPlanningService {
    ...
    public static Future<ICraftingPlan> submit(
        ...
        return submit(snapshot, strategy, lease, null, ae2Fallback);
    }
    ...
    public static Future<ICraftingPlan> submit(
        ...
        FutureTask<ICraftingPlan> task =
                new FutureTask<>(() -> {
                    try (var diagnosticScope =
                            ECOPlanningFailureDiagnostics.bindRequest(snapshot.diagnosticRequestId())) {
                        ...
                        ECOPlannerNoticeDispatcher.sendCycleDiagnostics(
                                noticeTarget, ECOCyclePlanningDiagnostics.EMPTY);
                        ...
                        try {
                            ECOSolveBudget budget =
                                    NEConfig.debugECOPlanner ? lease.budget().forDebug() : lease.budget();
                            ...
                            try {
                                ...
                                ECOPlanningFailureDiagnostics.logFailure(
                                        ...
                                        failure);
                                LOGGER.debug(
                                        "ECO planning failed; the caller will use AE2 crafting calculation", failure);
                            }
                            if (cancellationRequested.get()
                            ...
                            if (attempt.overflowBytes() != null) {
                                ...
                                ECOPlannerNoticeDispatcher.sendOverflow(
                                        noticeTarget, elapsedNanos, attempt.overflowBytes());
                            ...
                            if (ecoPlan.isPresent()) {
                                ...
                                diagnosticResult =
                                        simulation ? "eco_missing_sources_simulation" : "eco_executable_plan";
                                return ecoPlan.get();
                            }
                            ECOPlanningFailureDiagnostics.logTiming(
                                    ECOPlanningFailureDiagnostics.Stage.ENTRY,
                            ...
                            if (cancellationRequested.get()
                                ...
                                throw new CancellationException(
                                        "ECO crafting planning was cancelled before AE2 fallback");
                            }
                            try {
                            ...
                            lease.close();
                            ECOPlanningFailureDiagnostics.endRequest(snapshot.diagnosticRequestId(), diagnosticResult);
        ...
        try {
            ...
            if (plan.isEmpty()) {
                ECOPlanningFailureDiagnostics.logFailure(
                        ...
                        snapshot.requestedAmount(),
                        "assembler",
                        "resultStatus=" + result.status() + " expandedStates=" + result.expandedStates());
            }
            if (plan.isEmpty()
