...
public final class ECOExtractedPatternExecution {
    ...
    @Nullable private final String fastPathRejectionReason;
    ...
    static ECOExtractedPatternExecution ofNormalizedComponents(
        ...
        return new ECOExtractedPatternExecution(
                ...
                patternEligibility,
                key,
                key == null ? "KEY_BUILD_FAILED" : null);
    }
    ...
    @Nullable private static String findFastPathRejectionReason(
            IPatternDetails details,
        ...
        String outputFailure = validationFailure("OUTPUT", outputs, true, resultValidation);
        if (outputFailure != null) return outputFailure;
        String remainderFailure = validationFailure("REMAINDER", containers, false, resultValidation);
        if (remainderFailure != null) return remainderFailure;
        return validationFailure("INPUT", inputs, false, ECOFastPathStacks.ItemStackValidation.FAST_PATH_INPUT);
    }

    private static ECOPatternEligibility inspectPatternEligibility(
            IPatternDetails details, ECORecipeClassifier.Classification classification) {
        ...
        boolean structurallySupported =
                classification.supported() || "REMAINDER_IS_NOT_REUSABLE_ITEM".equals(classification.reason());
        return new ECOPatternEligibility(
                structurallySupported, inputTypes, structurallySupported ? "" : classification.reason());
    }

    @Nullable private static String validationFailure(
            String role,
            List<GenericStack> stacks,
            boolean requireNonEmpty,
            ECOFastPathStacks.ItemStackValidation validation) {
        ECOFastPathStacks.ItemStackValidationFailure failure =
                ECOFastPathStacks.validateItemStacks(stacks, Integer.MAX_VALUE, requireNonEmpty, validation);
        return failure == ECOFastPathStacks.ItemStackValidationFailure.NONE ? null : role + "_" + failure.name();
    }
