...
public final class SinglePatternGrowthCycleSolver implements CycleSolver {
    ...
    public CycleSolveResult solve(CycleSolveRequest request, ECOCancellation cancellation) throws InterruptedException {
        ...
        return fallback.solve(request, cancellation)
                .withAdditionalDiagnostics(List.of(new CycleSolveDiagnostic(
    ...
    private static CycleSolveResult adopt(SinglePatternGrowthResult growth) {
        ...
        diagnostics.add(
                new CycleSolveDiagnostic(CycleSolveDiagnostic.Code.SINGLE_PATTERN_NET_GROWTH, growth.diagnostic()));
        ...
        diagnostics.add(new CycleSolveDiagnostic(
                ...
                growth.seedShortfall().isEmpty()
                        ...
                        : "Exact firing vector is known, but start-up seed is short by "
                                + growth.exactSeedShortfall()));
        ...
        CycleSolveStatus status = growth.status() == SinglePatternGrowthStatus.SUCCESS
                ? CycleSolveStatus.SUCCESS
                : growth.status() == SinglePatternGrowthStatus.UNREPRESENTABLE
        ...
        Map<IPatternDetails, Long> runtimeTimes = growth.status() != SinglePatternGrowthStatus.UNREPRESENTABLE
                ...
                ? Map.of(details, growth.exactFirings().longValueExact())
                : Map.of();
        return new CycleSolveResult(
                status,
                ExecutionCountKnowledge.EXACT,
                Map.of(details, growth.exactFirings()),
                runtimeTimes,
