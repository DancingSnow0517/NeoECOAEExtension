...
public final class ECORecipeClassifier {
    ...
    public static Classification classify(IPatternDetails pattern) {
        ...
        try {
            ...
            if (outputs == null || outputs.length == 0) return unsupported("NO_OUTPUTS");
            ...
            for (IPatternDetails.IInput input : inputs) {
                ...
                if (ItemStack.isSameItem(inputStack, remainingStack)) {
                    ...
                    if (!selected.what().equals(remainingKey)
                            ...
                            && remainingStack.isDamageableItem()) {
            ...
            if (durabilityMutation) {
                return new Classification(Type.DURABILITY_MUTATION, true, "RUNTIME_SIMULATION_REQUIRED");
            }
            if (reusableComponent) {
                return new Classification(Type.REUSABLE_COMPONENT, true, "ONE_TO_ONE_REUSABLE_ITEM_OR_COMPONENT");
            }
            if (hasRemainder) {
            ...
            return unsupported(
                    "CLASSIFIER_FAILED:" + failure.getClass().getSimpleName().toUpperCase(Locale.ROOT));
        }
    }
