...
public record PlanTraceNode(
    ...
    public PlanTraceNode(
        ...
        this(
                ...
                BigInteger.valueOf(fromInventory),
                BigInteger.valueOf(toCraft),
                BigInteger.valueOf(missing),
                BigInteger.valueOf(firingCount));
    }

    public PlanTraceNode withExact(
            BigInteger requested,
            BigInteger fromInventory,
            BigInteger toCraft,
            BigInteger missing,
            BigInteger firingCount) {
        return new PlanTraceNode(
                kind,
                key,
                pattern,
                compatibilityLong(requested),
                compatibilityLong(fromInventory),
                compatibilityLong(toCraft),
                compatibilityLong(missing),
                compatibilityLong(firingCount),
                selection,
                reason,
                requested,
                fromInventory,
