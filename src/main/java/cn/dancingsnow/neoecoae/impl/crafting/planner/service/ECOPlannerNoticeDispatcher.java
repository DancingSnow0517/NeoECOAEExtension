...
public final class ECOPlannerNoticeDispatcher {
    ...
    public static void sendOverflow(@Nullable Target target, long elapsedNanos, java.math.BigInteger exactBytes) {
        send(target, ECOPlannerFallbackReason.OVERFLOW, elapsedNanos, ByteAmountFormatter.format(exactBytes));
    }

    private static void send(
            @Nullable Target target, ECOPlannerFallbackReason reason, long elapsedNanos, String formattedBytes) {
        if (target == null) {
            return;
        }
        ...
        server.execute(() -> {
            ...
            ECOPlannerNetwork.sendToPlayer(
                    player,
                    new ECOPlannerNoticePayload(
                            target.containerId(), reason.id(), Math.max(0L, elapsedNanos), formattedBytes));
        });
    }

    public static void sendCycleDiagnostics(@Nullable Target target, ECOCyclePlanningDiagnostics diagnostics) {
        if (target == null) {
            return;
        }
        MinecraftServer server = target.player().getServer();
        ...
        server.execute(() -> {
            ...
            ECOPlannerNetwork.sendToPlayer(player, new ECOCycleDiagnosticsPayload(target.containerId(), diagnostics));
        });
    }
