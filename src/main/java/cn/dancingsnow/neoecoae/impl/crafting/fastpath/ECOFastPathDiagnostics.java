...
public final class ECOFastPathDiagnostics {
    ...
    public static void logSlowPath(
            ECOExtractedPatternExecution execution, ECOFastPathFallbackReason reason, BlockPos workerPos, long tick) {
    ...
    public static void logBatchFailure(
        ...
        logFailure(request.details(), reason, stage, ownerPos, tick, "batch=" + request.batchSize() + " " + context);
    }
    ...
    public static void logCpuReservation(ICraftingPlan plan, KeyCounter reservedItems, BlockPos ownerPos, long tick) {
        if (!NEConfig.debugEcoFastPath) {
            return;
        }
    ...
    private static PatternDescription describe(IPatternDetails details) {
        ...
        String definition = identity instanceof AEKey key
                ? describeKey(key)
        ...
        return new PatternDescription(
                definition, identityHash, primaryOutput, details.getClass().getName());
    }
    ...
    private static ECOFastPathStage stageFor(ECOFastPathFallbackReason reason) {
        return switch (reason) {
            case FAST_PATH_DISABLED,
                    ...
                    UNSAFE_EXPECTED_OUTPUT,
                    UNSAFE_CONTAINER_ITEM,
            ...
            case RUNTIME_STACK_CONVERSION_FAILED,
                    OUTPUT_MISMATCH,
                    CONTAINER_MISMATCH,
                    INPUT_MISMATCH,
                    CACHE_VALIDATION_REJECTED -> ECOFastPathStage.CACHE_VERIFY;
    ...
    private record PatternDescription(
            String definition, int identityHash, String primaryOutput, String implementation) {}
