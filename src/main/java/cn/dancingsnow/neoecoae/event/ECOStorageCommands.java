...
public final class ECOStorageCommands {
                ...
                .then(Commands.literal("storage")
                        .then(Commands.literal("migrate")
                                ...
                                .then(Commands.argument("uuid", UuidArgument.uuid())
                                        ...
                                        .executes(context ->
                                                migrate(context.getSource(), UuidArgument.getUuid(context, "uuid")))))
                        .then(Commands.literal("recover")
                                .requires(source -> source.hasPermission(2))
                                .then(Commands.argument("uuid", UuidArgument.uuid())
                                        .suggests(ECOStorageCommands::suggestDomainIds)
                                        .executes(context ->
                                                recover(context.getSource(), UuidArgument.getUuid(context, "uuid")))))
                        .then(Commands.literal("diagnose")
                                .requires(source -> source.hasPermission(2))
                                .then(Commands.argument("uuid", UuidArgument.uuid())
                                        .suggests(ECOStorageCommands::suggestDomainIds)
                                        .executes(context ->
                                                diagnose(context.getSource(), UuidArgument.getUuid(context, "uuid"), 1))
    ...
    private static int migrate(CommandSourceStack source, UUID domainId) {
        ...
        source.sendFailure(Component.literal("V1 archive migration failed: "
    ...
    private static void reportRecoveryFailure(
            CommandSourceStack source, UUID domainId, ECOInfiniteStorageEngine engine) {
        ...
        source.sendFailure(Component.empty()
                ...
                .append(Component.literal(engine.getFailureReason().orElse("未提供详细原因"))
    ...
    private static boolean reportOrphanedEntries(
            CommandSourceStack source, UUID domainId, ECOInfiniteStorageEngine engine, int page) {
        List<ECOInfiniteStorageEngine.OrphanedStack> entries = engine.getOrphanedStacks().stream()
                .sorted(Comparator.comparing(
                        stack -> describeEncodedKey(stack.encodedKey()).id()))
                .toList();
        if (entries.isEmpty()) {
            return true;
        }
        ...
        if (page > totalPages) {
            source.sendFailure(Component.literal("诊断页码超出范围：第 " + page + " 页不存在，当前共有 " + totalPages + " 页。")
        ...
        source.sendSuccess(
                () -> Component.empty()
                        .append(Component.literal("域：").withStyle(ChatFormatting.GRAY))
                        .append(Component.literal(domainId.toString()).withStyle(ChatFormatting.DARK_GRAY))
                        .append(Component.literal("  第 " + page + " / " + totalPages + " 页")
        ...
        for (int index = firstEntry; index < lastEntry; index++) {
            ...
            source.sendSuccess(
                    () -> Component.empty()
                            .append(Component.literal(" - [" + description.type() + "] ")
                            ...
                            .append(Component.literal(entry.amount().toString()).withStyle(ChatFormatting.GOLD)),
                    false);
        }
        if (page < totalPages) {
            source.sendSuccess(
                    () -> Component.literal("使用 /neoecoae storage diagnose " + domainId + " " + (page + 1) + " 查看下一页。")
                            .withStyle(ChatFormatting.GRAY),
                    false);
        }
        return true;
    ...
    private static int ignoreMissing(CommandSourceStack source, UUID domainId) {
        ...
        if (engine.getState() != ECOInfiniteDomainState.READY) {
            source.sendFailure(Component.literal("Infinite storage is not ready: "
                    + engine.getFailureReason().orElse(engine.getState().name())));
            return 0;
        }
    ...
    private static CompletableFuture<Suggestions> suggestDomainIds(
        ...
        return SharedSuggestionProvider.suggest(
                ECOInfiniteStorageDomains.findExistingDomainIds(
