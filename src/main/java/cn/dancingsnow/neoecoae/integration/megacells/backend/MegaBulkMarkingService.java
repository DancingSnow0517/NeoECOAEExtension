...
public final class MegaBulkMarkingService {
    ...
    public static boolean hasBulkCell(ECOStorageSystemBlockEntity host) {
        for (ECODriveBlockEntity drive : host.getStorageDrivesForIntegration()) {
            ...
            if (stack != null && !stack.isEmpty() && stack.getItem() instanceof ECOMegaLongBulkStorageCellItem) {
                return true;
            }
        }
        return false;
    ...
    public static ItemStack normalizeMarker(ItemStack stack) {
        ...
        if (key == null || chainFor(key).isEmpty()) return ItemStack.EMPTY;
    ...
    public static boolean isSameMarkerChain(ItemStack left, ItemStack right) {
        ...
        CompressionChain leftChain = chainFor(leftKey);
        return !leftChain.isEmpty() && leftChain.equals(chainFor(rightKey));
    }
    ...
    private static List<TargetCell> collectTargetCells(
            ECOStorageSystemBlockEntity host, List<ECODriveBlockEntity> drives) {
    ...
    private static MarkResult markCandidates(
        ...
        for (TargetCell target : targets) {
            var config = ((ECOMegaLongBulkStorageCellItem)
                            target.drive().getCellStack().getItem())
        ...
        for (Candidate candidate : rawCandidates) {
            if (uniqueCandidates.stream().noneMatch(existing -> sameChain(existing.chain(), candidate.chain()))) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            cellItem.getConfigInventory(cellStack)
                    .setStack(
                            slotTarget.slot(),
                            new GenericStack(accepted.get(index).key(), 0L));
    ...
    private static long transferMarkedChains(
            ECOStorageSystemBlockEntity host, List<ECODriveBlockEntity> drives, List<TargetCell> targets) {
    ...
    private static long transfer(
        ...
        if (inserted < extracted) {
            ...
            if (restored != remainder) {
                throw new IllegalStateException("Internal-transfer rollback incomplete: " + restored + "/" + remainder);
            }
        }
        return inserted;
    }
    ...
    private static CompressionChain chainFor(AEItemKey key) {
    ...
    private record TargetCell(ECODriveBlockEntity drive, ECOMegaLongBulkStorageCell storage) {}
