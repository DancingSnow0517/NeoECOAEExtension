...
public final class NEMegaItems {
    static {
        ...
        REGISTRATE.addLang(
                "tooltip",
                NeoECOAE.id("megacells.configure_item"),
                "Configure 25 compression chains, or 50 with an ECO MEGA Upgrade Card; the first variant selects the storage form");
        REGISTRATE.addLang(
                "tooltip",
                NeoECOAE.id("megacells.compression_builtin"),
                "Compression variants are enabled by default; no upgrade card required");
        REGISTRATE.addLang(
                "tooltip", NeoECOAE.id("megacells.empty_only"), "Only an empty storage matrix can be disassembled");
        REGISTRATE.addLang("tooltip", NeoECOAE.id("megacells.housing"), "MEGA Cells housing for %s storage matrices");
    }
    ...
    public static final ItemEntry<Item> ECO_MEGA_UPGRADE_CARD = REGISTRATE
            .item("eco_mega_upgrade_card", Upgrades::createUpgradeCardItem)
    ...
    public static final ItemEntry<ECOMegaLongBulkStorageCellItem> ECO_MEGA_LONG_BULK_CELL = REGISTRATE
            .item(
                    "eco_mega_long_bulk_cell",
                    p -> new ECOMegaLongBulkStorageCellItem(
                            p.stacksTo(1).rarity(Rarity.EPIC), ECOTier.L9, NEMegaCellTypes.MEGA_ITEM))
    ...
    static ItemEntry<Item> housing(String family, String displayFamily) {
        return REGISTRATE
                .<Item>item(family + "_cell_housing", p -> new MegaCellHousingItem(p, "cell_type.neoecoae." + family))
    ...
    private static ItemEntry<ECOMegaItemStorageCellItem> itemCell(String size, long capacity, Rarity rarity) {
        return REGISTRATE
                .item(
                        "eco_mega_item_cell_" + size,
                        p -> new ECOMegaItemStorageCellItem(
                                p.stacksTo(1).rarity(rarity), ECOTier.L9, NEMegaCellTypes.MEGA_ITEM, capacity))
    ...
    private static ItemEntry<ECOMegaFluidStorageCellItem> fluidCell(String size, long capacity, Rarity rarity) {
        return REGISTRATE
                .item(
                        "eco_mega_fluid_cell_" + size,
                        p -> new ECOMegaFluidStorageCellItem(
                                p.stacksTo(1).rarity(rarity), ECOTier.L9, NEMegaCellTypes.MEGA_FLUID, capacity))
    ...
    static <T extends ECOStorageCellItem> ItemEntry<T> optionalCell(
            ...
            long capacity,
            Rarity rarity,
            CellFactory<T> factory,
            Supplier<ECOCellType> type) {
        return REGISTRATE
                .item(
                        "eco_" + family + "_cell_" + size,
