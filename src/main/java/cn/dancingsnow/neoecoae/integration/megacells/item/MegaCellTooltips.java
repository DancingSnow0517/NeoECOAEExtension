...
final class MegaCellTooltips {
    ...
    static void append(ECOStorageCellItem cell, List<Component> lines) {
        lines.add(Component.translatable(
                        "tooltip.neoecoae.megacells.storage_type",
                        cell.getCellType().desc())
                .withStyle(ChatFormatting.DARK_GRAY));

        if (cell.getBytes() == MegaCellCapacities.MEGA_4G_CAPACITY) {
            lines.add(Component.translatable("tooltip.neoecoae.megacells.4g").withStyle(ChatFormatting.LIGHT_PURPLE));
