...
public final class StorageBulkMarkingIntegration {
    ...
    public static ItemStack normalizeMarker(ItemStack stack) {
        return handler == null || stack == null || stack.isEmpty() ? ItemStack.EMPTY : markerNormalizer.apply(stack);
    }
    ...
    public static boolean isSameMarkerChain(ItemStack left, ItemStack right) {
        return handler != null
                ...
                && !left.isEmpty()
                && !right.isEmpty()
                && sameMarkerChain.test(left, right);
    }
    ...
    public record MarkResult(
            Status status, int added, int alreadyMarked, int notCompressible, int noSpace, long transferred) {}
