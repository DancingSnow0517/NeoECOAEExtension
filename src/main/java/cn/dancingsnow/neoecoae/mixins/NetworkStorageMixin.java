...
public abstract class NetworkStorageMixin implements ECOBigIntegerStorage {
    ...
    public BigInteger insertBigInteger(AEKey what, BigInteger amount, Actionable mode, IActionSource source) {
        ...
        if (amount.compareTo(MAX_LONG) <= 0) {
            return checked(
                    amount,
                    BigInteger.valueOf(
                            ((MEStorage) (Object) this).insert(what, amount.longValueExact(), mode, source)));
        }
        ...
        BigInteger accepted = checked(
                MAX_LONG, BigInteger.valueOf(((MEStorage) (Object) this).insert(what, Long.MAX_VALUE, mode, source)));
                ...
                for (MEStorage inventory : inventoryList) {
                    ...
                    if (inventory.isPreferredStorageFor(what, source)) {
                        remaining = remaining.subtract(
                                checked(remaining, exact.insertBigInteger(what, remaining, mode, source)));
                    } else {
    ...
    private static BigInteger simulateMount(
            MEStorage inventory, AEKey what, BigInteger remaining, long ordinaryBudget, IActionSource source) {
        ...
        return checked(
                offered,
                BigInteger.valueOf(inventory.insert(what, offered.longValueExact(), Actionable.SIMULATE, source)));
    }

    private static long consumeBudget(long budget, BigInteger accepted) {
        return accepted.compareTo(BigInteger.valueOf(budget)) >= 0 ? 0L : budget - accepted.longValueExact();
    }
