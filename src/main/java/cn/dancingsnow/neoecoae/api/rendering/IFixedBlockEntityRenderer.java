...
public interface IFixedBlockEntityRenderer<T extends BlockEntity> {
    ...
    default void tessellateModelWithAO(
        ...
        tessellateModelWithAO(
                level, model, state, pos, poseStack, bufferSource, RenderType.cutout(), random, packedOverlay);
    }
    ...
    default void tessellateModelWithAO(
        ...
        modelRenderer.tesselateWithAO(
                level, bakedModel, state, pos, poseStack, vertexConsumer, false, random, 42, packedOverlay);
    }
    ...
    default void tessellateModel(
            ...
            int packedLight,
            int packedOverlay) {
        tessellateModel(owner, poseStack, bufferSource, model, packedLight, packedOverlay, RenderType.cutout());
    }
    ...
    default void renderQuadsWithoutAO(
            PoseStack poseStack, VertexConsumer buffer, List<BakedQuad> quads, int packedLight, int packedOverlay) {
        for (BakedQuad quad : quads) {
            buffer.putBulkData(poseStack.last(), quad, 1, 1, 1, packedLight, packedOverlay);
        }
    }
}
