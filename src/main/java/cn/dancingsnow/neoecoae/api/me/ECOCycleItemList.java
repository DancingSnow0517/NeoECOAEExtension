...
public record ECOCycleItemList(List<Entry> items) implements PacketWritable {
    ...
    public ECOCycleItemList(FriendlyByteBuf data) {
    ...
    public record Entry(
            AEKey what,
            BigInteger exactConsumed,
            BigInteger exactProduced,
            BigInteger exactSingleNetOutput,
            BigInteger exactTotalNetOutput,
        ...
        public Entry(AEKey what, long singleNetOutput, long totalNetOutput, int componentId) {
            this(
                    ...
                    BigInteger.ZERO,
                    BigInteger.ZERO,
                    BigInteger.valueOf(singleNetOutput),
        ...
        public Entry(AEKey what, long singleNetOutput, long totalNetOutput, boolean known, int componentId) {
            this(
                    ...
                    known ? CycleSolveStatus.SUCCESS : CycleSolveStatus.NOT_IMPLEMENTED,
                    componentId);
        }

        public long singleNetOutput() {
            return exactSingleNetOutput.longValue();
        }

        public long totalNetOutput() {
            return exactTotalNetOutput.longValue();
        }

        public boolean totalNetOutputKnown() {
        ...
        public boolean isCycleProduct() {
            return exactTotalNetOutput.signum() > 0;
        }

        public BigInteger displayedTotal() {
            return isCycleProduct() ? exactProduced : exactConsumed;
        }
    }

    private static void writeBigInteger(FriendlyByteBuf data, BigInteger value) {
