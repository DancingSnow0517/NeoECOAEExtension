...
public final class ECOBatchDispatchContext {
    ...
    @Nullable private ECOExtractedPatternExecution execution;

    public ECOBatchDispatchContext(
            IPatternDetails pattern,
            List<List<GenericStack>> inputs,
            List<GenericStack> outputs,
            List<GenericStack> containerItems,
            Level level,
    ...
    public List<GenericStack> containerItems() {
    ...
    public @Nullable UUID craftingJobId() {
        return craftingJobId;
    }

    public static ECOBatchDispatchContext create(
            IPatternDetails pattern,
            KeyCounter[] inputs,
            KeyCounter outputs,
            ...
            Level level,
            @Nullable UUID jobId) {
        return new ECOBatchDispatchContext(
                pattern,
                java.util.Arrays.stream(inputs)
                        .map(ECOFastPathStacks::copyCounter)
                        .toList(),
                ECOFastPathStacks.copyCounter(outputs),
                ECOFastPathStacks.copyCounter(containers),
                level,
                jobId);
    }
    ...
    public ECOExtractedPatternExecution execution() {
        ...
        if (result == null) {
            result = ECOExtractedPatternExecution.create(
                    pattern, inputCounters(), outputCounter, containerCounter, level);
            execution = result;
        }
        return result;
    }
