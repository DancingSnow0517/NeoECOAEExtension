...
public record CraftingCapabilitySnapshot(
    ...
    public CraftingCapabilitySnapshot {
        ...
        activeFxCount = NEMath.clamp(activeFxCount, 0, physicalFxCount);
        ...
        theoreticalOverclock = NEMath.clamp(theoreticalOverclock, 0, MAX_OVERCLOCK);
        effectiveOverclock = NEMath.clamp(effectiveOverclock, 0, theoreticalOverclock);
    ...
    public static CraftingCapabilitySnapshot calculate(Input input) {
        ...
        long multiplierLong = NEMath.saturatingAdd(
                NEMath.saturatingMultiply(normalHosts, 2L), NEMath.saturatingMultiply(highEnergyHosts, 8L));
    ...
    static int calculateOverclock(long ftParallelCapacity, long baseFxCapacity) {
        ...
        return (int) NEMath.clamp(Math.round(((double) overflow / (double) ft) / 0.05D), 0L, MAX_OVERCLOCK);
    }
