...
<ItemLink id="neoecoae:crafting_worker" /> 提供可同时运行的独立任务槽。x1 状态下每个 FX 工作核心提供 **1 个**任务槽；网络交换生效时，每个本地主机内的 FX 工作核心按已连接 F 主机数提供任务槽。x2/x8 只提升单槽批量，不增加任务槽。
...
| 项目 | 普通网络交换 | 高能网络交换 |
|------|--------------|--------------|
| 合成倍率 | 每个任务槽处理 **x2** 合成数量 | 每个任务槽处理 **x8** 合成数量 |
| FX 任务槽 | 每个本地主机内的 FX 工作核心按每台已连接 F 主机提供 1 个任务槽；x2 只提升单槽批量 | 每个本地主机内的 FX 工作核心按每台已连接 F 主机提供 1 个任务槽；x8 只提升单槽批量 |
| 耗电倍率 | **x4** | **x16** |
| 冷却要求 | 网络冷却池可提供合格冷却液 | 网络冷却池可提供最高级冷却液，最高支持超频 9 |
| Tick 冷却 | 只有已生效的网络交换任务按 tick 冷却，每条活跃任务线程每 tick 消耗 **4** 冷却值；单槽批量不参与计算 | 只有已生效的网络交换任务按 tick 冷却，每条活跃任务线程每 tick 消耗 **16** 冷却值；单槽批量不参与计算 |
...
| 模块混用 | 两种模块可以混用，每台主机按自身模块档位贡献容量。 |
| 能耗与界面 | 交换生效时，每台主机按全部可用 FX 线程持续拉满额定耗电；供电不足时相关任务暂停，GUI 显示聚合能耗。 |
| 共享控制 | 网络成员共用一套 GUI、超频开关和主动冷却开关，GUI 同时显示聚合能耗并控制主动冷却。 |
| 冷却池 | 开启主动冷却后，所有成员的冷却值缓存合并为一个网络冷却池，消耗在满足冷却等级的成员间轮转均摊，不要求执行任务的主机单独持有冷却液。 |
| 冷却不足 | 冷却池无法支付当前 tick 消耗时，相关任务暂停；补充冷却值后从原进度继续。冷却液不满足模块要求时，对应倍率按 **x1** 处理。 |

#### 完整八主机虚拟合成

当同一个逻辑合成交换网络**恰好**连接 **8 台 F9 合成主机**，每台均安装高能网络交换模块、扩展到当前配置允许的最大长度（默认每台 **88** 个任务槽），并开启主动冷却后，网络才会进入虚拟合成模式。普通模块、任一非 F9 主机或未满配的主机会使网络保持原有合成行为。

- 每条 FX 工作核心任务线程承载一种配方任务，并一次接收该任务的全部剩余合成数量。
- 输入和输出以物品键和 64 位数量汇总，不在 FX 工作核心内展开或保存为实际物品堆。
- 虚拟任务在工作核心的首个 tick 完成并返回全部汇总输出。
- 每条活跃虚拟任务线程固定从共享池中消耗 **10000 mB 冷却液/tick**；共享池无法一次支付完整消耗时，任务等待。
- 支持数量遵循有符号 64 位范围；达到 `Long.MAX_VALUE` 的饱和值在 Crafting Status 提示中显示为**无限**。
...
- 每个 FX 工作核心在 x1 状态下提供 1 个任务槽；网络交换生效时，每个本地主机内的 FX 工作核心按已连接 F 主机数提供任务槽，x2/x8 只提升单槽批量
