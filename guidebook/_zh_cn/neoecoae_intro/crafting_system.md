...
<ItemLink id="neoecoae:crafting_worker" /> 执行并行核心提供的任务槽。每个并行核心固定提供 **2 个**可同时运行的独立任务槽。
...
并行核心（<ItemLink id="neoecoae:crafting_parallel_core_l4" />、<ItemLink id="neoecoae:crafting_parallel_core_l6" /> 或 <ItemLink id="neoecoae:crafting_parallel_core_l9" />）提供任务槽和单槽批量：每个核心提供 2 个任务槽，核心等级决定每个槽一次可处理的合成数量。
...
- 交换倍率不会增加任务槽：每台物理主机独立管理自己的核心与槽位，网络总槽位为所有主机槽位之和
...
- 只有已生效的网络交换任务改用按 tick 冷却：普通模块每个活跃任务槽每 tick 消耗 **1** 冷却值，高能模块消耗 **4**；单槽批量不参与计算
...
- 每个并行核心增加 2 个可同时运行的任务槽
- 更高等级的并行核心增加每个任务槽一次处理的物品数量
