...
| 工作线程 | 每个 FX 工作核心按每台参与交换的 F 主机提供 1 条线程 | 每个 FX 工作核心按每台参与交换的 F 主机提供 1 条线程 |
| 耗电倍率 | **x4** | **x16** |
| 冷却要求 | 网络冷却池可提供合格冷却液 | 网络冷却池可提供最高级冷却液，最高支持超频 9 |
| Tick 冷却 | 只有已生效的网络交换任务按 tick 冷却，每条活跃任务线程每 tick 消耗 **4** 冷却值；单槽批量不参与计算 | 只有已生效的网络交换任务按 tick 冷却，每条活跃任务线程每 tick 消耗 **16** 冷却值；完整虚拟交换见下文 |
...
当同一网络中恰好有 **8 台**已连接的 F9 主机，并形成有效的 **x8** 高能网络交换时，符合 FastPath 条件的任务会进入虚拟合成。一次提交会以 `long` 数量接收全部可用任务量：消耗多少输入，就生成对应数量的输出，不再受普通单槽批量上限限制。

每条活跃虚拟任务线程每 tick 固定从共享冷却池消耗 **10,000 mB** 冷却液。该消耗按任务线程计算，不随物品数量或批量变化。x8 网络少于 8 台主机时仍使用普通 x8 行为，保留原有单槽批量限制与每条活跃任务线程每 tick **16** 冷却液的消耗。
...
- 需要在输入仓中放入冷却剂流体
- 可在JEI中查看冷却剂配方
- 系统会将冷却剂转换为冷却值缓冲；x1 模式在工作核心接收样板时按合成次数消耗，已生效的 x2/x8 网络交换任务按活跃槽位每 tick 消耗；完整 8 主机的虚拟 x8 任务每条活跃任务线程每 tick 固定消耗 10,000 mB
